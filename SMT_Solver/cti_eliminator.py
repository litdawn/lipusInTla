...
def make_indquickcheck_tla_spec(invs, sat_invs_group, orig_k_ctis, quant_inv_fn, tla_ins):
    begin = time.time()
...
def make_ctiquickcheck_cfg(seed_tmpl):
    ...
    invcheck_tla_indcheck_cfg += seed_tmpl.model_const
    invcheck_tla_indcheck_cfg += "\n"
...
def eliminate_ctis(invs, orig_k_ctis, seed_tmpl):
    ...
    logging.info(
        "Starting iteration %d of eliminate_ctis (min_conjs=%d, max_conjs=%d)" % (iteration, min_conjs, max_conjs))
    ...
    while curr_ind < len(inv_list):
        ...
        for ci, cti_chunk in enumerate(cti_chunks):
            ...
            spec_str = make_indquickcheck_tla_spec(invs, sat_invs_group, cti_chunk, quant_inv_fn, tla_ins)
