...
class CEEmbedding(nn.Module):
    def __init__(self, vars):
        ...
        self.RNNs = nn.LSTM(config.SIZE_PCA_NUM, config.SIZE_EXP_NODE_FEATURE, 2)
    ...
    def forward(self, CE):
        ...
        weis = torch.cosine_similarity(emb, self.attvec).reshape([1, 1])  # 仅计算正例部分的相似度
        swis = self.softmaxer(weis)
        ce_emb = torch.mm(swis, emb)
